...
function IsAdmin() : boolean {
    fetch("http://localhost:8000/api/user/admin")
        .then(res => res
            .json()).then(isAdmin => {
                return isAdmin;
    })
...
export default function Page() {
    ...
    const {username, description, profilePicture, bannerPicture} = useUser();
    ...
    useEffect(() => {
        bannerImage.current!.src = bannerUrl;
        if(bannerPicture !== "http://localhost:8000" && bannerPicture !== ""){
            bannerImage.current!.src = bannerPicture;
        }
        profileImage.current!.src = profileUrl;
        if(profilePicture !== "http://localhost:8000" && profilePicture !== ""){
            profileImage.current!.src = profilePicture;
        }
    ...
    return (
        <div className="flex flex-col items-center bg-background text-palePurple h-full overflow-auto"
             style={{width: "100vw", height: "80vh"}}>
            <div className="w-full relative group">
                <img ref={bannerImage} src={bannerUrl} alt="banner"
                     className="w-full h-56 shadow-lg shadow-foreground cursor-pointer"
                     onClick={() => setShowBannerInput(true)}/>
                <div
                    className="absolute top-2 right-2 bg-white text-black px-2 py-1 rounded shadow-md opacity-0 group-hover:opacity-100 cursor-pointer"
                    onClick={() => setShowBannerInput(true)}>Edit
                </div>
                {showBannerInput && (
                    <div
                        ...
                        <button onClick={() => setShowBannerInput(false)}
                                className="ml-2 bg-white text-black px-2 py-1 rounded">Close
                        </button>
                    </div>
                )}
            </div>
            <div className="relative w-full bg-violet shadow shadow-foreground">
                <div className="absolute left-1/2 transform -translate-x-1/2 -translate-y-1/2 group">
                    <img ref={profileImage} src={profileUrl} alt="profile picture"
                         className="h-24 w-24 rounded-full border-4 border-palePurple cursor-pointer shadow-foreground"
                         onClick={() => setShowProfileInput(true)}/>
                    <div
                        className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white text-black px-2 py-1 rounded shadow-md opacity-0 group-hover:opacity-100 cursor-pointer"
                        onClick={() => setShowProfileInput(true)}>Edit
                    </div>
                    {showProfileInput && (
                        <div
                            className="absolute top-0 left-0 w-full h-full flex items-center justify-center bg-black bg-opacity-50">
                            <input type="file" accept="image/*" onChange={handleProfileChange} className="p-2 rounded"/>
                            <button onClick={() => setShowProfileInput(false)}
                                    className="ml-2 bg-white text-black px-2 py-1 rounded">Close
                            </button>
                        </div>
                    )}
            ...
            <div className="w-2/3 p-4 bg-violet shadow-sm shadow-foreground mt-4 relative">
                ...
                {isEditingDescription ? (
                    <div>
                        <textarea value={newDescription} onChange={handleDescriptionChange}
                                  className={"w-full p-2 rounded text-background"} style={{boxSizing: "border-box"}}/>
                        <button onClick={saveDescription} className="mt-2 bg-white text-black px-2 py-1 rounded">Save
                        </button>
                    </div>
                ) : (
                    <div>
                        <div ref={descriptionText} className="text-palePurple mt-2 break-words">{description}</div>
                        <button onClick={() => setIsEditingDescription(true)}
                                className="absolute top-2 right-2 bg-white text-black px-2 py-1 rounded shadow-md">Edit
                            Description
                        </button>
                    </div>
                )}
            </div>
            <div className="w-2/3 p-4 bg-violet shadow-sm shadow-foreground mt-4">
